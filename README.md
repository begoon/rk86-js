...
| `?handoff=<uuid>` | забрать бинарник из `localStorage["asm8-handoff:<uuid>"]` и запустить |
...
Для больших программ data URL не помещается в адресную строку (~8 КБ в Chrome, ошибка 431). Ассемблер пишет `{ts, url}` в `localStorage["asm8-handoff:<uuid>"]`, открывает эмулятор с `?handoff=<uuid>`; эмулятор читает и **удаляет** ключ (one-shot). Старые ключи (>1 часа) подметаются при следующей записи.
